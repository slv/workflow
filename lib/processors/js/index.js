    ...
    target.jshintSrc.forEach(function (file) {
      ...
      if (!jshint.JSHINT(code, jshintrcConfig)) {
        ...
        if (!jshint.JSHINT.errors.every(function (el) { return (el.raw.match(/is not defined\.$/) && globals[el.a]) })) {
          ...
          err.filename = file
          err.errors = []
          jshint.JSHINT.errors.forEach(function (el) { if (!el.raw.match(/is not defined\.$/) || !el.a || !globals[el.a]) err.errors.push(el) })
          callback(err)
        }
      }
      jshint.JSHINT.data().globals.forEach(function (el) {globals[el] = true; });
